...
export const AdminCarsScreen: React.FC = () => {
  ...
  const filteredCars = useMemo(
    () => (activeStatus === 'ALL' ? cars : cars.filter((car) => car.status === activeStatus)),
    [activeStatus, cars]
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<ScreenState type="empty" icon="car-outline" title="Không có xe" message="Không có xe phù hợp với bộ lọc hiện tại." />}
        renderItem={({ item }) => {
